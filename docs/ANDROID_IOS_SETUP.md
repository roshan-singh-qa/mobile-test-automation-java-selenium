...
> It is advised to keep emulator open and start tests against an already opened emulator to speed up tests. Starting emulator manually/by Appium is time consuming and will slow down the test execution.

## iOS
Make sure you have a Mac, otherwise it will not work (no legal way to use iOS on Windows/Linux).
